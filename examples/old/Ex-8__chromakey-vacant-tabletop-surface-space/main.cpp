...
int main(int argc, char* argv[])
{
    ...
    // initialize chromakey colors
     uint8_t blue[4] = { 180,117, 69, 1 };
     uint8_t green[4] = { 27, 120, 55, 1 };
    ...
    while (sptr_i3d->isRun()) {
        ...
        for(auto& index: indexClusters[0]){
            pCloud[points[index].m_id].setPixel_BGRA(green);
        }

        uint8_t imgData[ w * h * 4 ];
        for(int i = 0; i < pCloud.size(); i ++){
                utils::stitch(i, pCloud[i], imgData);
        }
        ...
        if (!dets.empty()) {
            for (size_t i = 0; i < dets[0].sizes()[0]; ++i) {
                ...
                cv::putText(frame,
                    ...
                    (right - left) / 200, cv::Scalar(255, 255, 255), 2);
            }
        }
